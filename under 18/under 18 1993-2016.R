...
obs_counts_df <- data.frame(
  Data_Frame_Name = data_frame_names,
...
#library(readr)
#write_csv(mean_CI_df, "C:/Users/aleong/Desktop/NAMCS/Replication/means_under18_1993-2016.csv")
...
ggplot(mean_CI_df, aes(x = year, y = mean, col = variable, shape = variable, fill=variable)) + #remove fill if grey
  ...
  labs(x = "Year", y = "Proportion of visit records \nfor children <18y (%)", col = "Variable", shape = "Variable") +
...
#results comparison (u18)
#1996: mine = 61% (95% CI, 54% - 67%)  paper = 61% (95% CI, 53% - 69%)
#2006: mine = 69% (95% CI, 63% - 76%)  paper = 71% (95% CI, 65% - 77%)
# could be because paper didn't exclude CHCs?
...
#results comparison (u18, NONPEDGEN)
#1996: mine = 29% (95% CI, 22% - 36%)  paper = 28% (95% CI, 22% - 35%)
#2006: mine = 23% (95% CI, 17% - 28%)  paper = 22% (95% CI, 17% - 27%) 
...
#tau = -0.621, 2-sided pvalue = 3.7882e-05 (u1)
