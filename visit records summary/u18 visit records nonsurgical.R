...
obs_counts_df <- obs_counts_df %>%
  rename(Observations_U18_nonsurgical = Observations_U18)
         

ggplot(data=obs_counts_df, aes(x=year, y=Observations_U18_nonsurgical))+
  geom_bar(stat="identity", colour="black", fill ="indianred2") +
  labs (title = "Number of non-surgical visit records over time, \npatients under age 18, NAMCS 1993-2019", x = "Survey year", y = "Number of records") +
  scale_x_continuous(breaks = seq(min(obs_counts_df$year), max(obs_counts_df$year), by = 2)) +
  scale_y_continuous(limits = c(0, 13000)) +
  geom_text(aes(label = Observations_U18_nonsurgical), vjust = -0.5, size=3)

#library(readr)
#write_csv(obs_counts_df, "u18nonsurgical.csv")
